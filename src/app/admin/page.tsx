        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {stats.unreadAlerts > 0 && (
                <motion.div
                    ...
                    className="bg-accent/10 border border-accent/20 p-4 rounded-[8px] flex items-center justify-between shadow-xl shadow-accent/5"
                >
                    ...
                    <button
                        ...
                        className="bg-primary text-white px-6 py-2 rounded-[8px] text-xs font-bold uppercase tracking-wider hover:bg-black transition-all shadow-lg font-inter"
                    >
                        View Transmissions
                    </button>
                </motion.div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <div className="flex items-center gap-4 mt-6">
                        <div className="bg-primary/5 px-4 py-2 rounded-[8px] border border-primary/5 flex items-center gap-3">
                            <Globe size={16} className="text-accent" />
                            <span className="text-xs font-bold text-primary uppercase tracking-wider font-inter">Global Traffic Active</span>
                        </div>
                        <p className="text-primary/40 font-bold text-xs uppercase tracking-widest bg-white px-3 py-1.5 rounded-[8px] border border-primary/5 shadow-sm font-inter">Last Synced: Just Now</p>
                    </div>
                </div>

                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 px-4 py-2 bg-white border border-primary/5 rounded-[8px] shadow-sm">
                    ...
                    <div className="flex items-center gap-2 px-4 py-2 bg-accent shadow-lg shadow-accent/20 rounded-[8px]">
                        <Award size={14} className="text-primary" />
                        <span className="text-xs font-bold text-primary uppercase tracking-wider font-inter">Premium Tier</span>
                    </div>
                    <button className="p-2.5 text-primary/30 hover:text-primary border border-primary/5 bg-white rounded-[8px] transition-all hover:shadow-md">
                ...
                <div className="flex items-center gap-4">
                    <div className="relative group">
                        ...
                        <input
                            ...
                            className="bg-white border border-primary/5 rounded-[8px] pl-12 pr-6 py-4 text-sm font-bold outline-none focus:ring-4 focus:ring-accent/10 focus:border-accent/30 transition-all w-80 shadow-sm font-inter"
                        />
                    </div>
                    <button className="flex items-center gap-3 px-6 py-4 bg-primary text-white rounded-[8px] text-xs font-bold uppercase tracking-wider hover:bg-black transition-all shadow-xl shadow-primary/20 group font-inter">
                    ...
                    {activeSection === "ACTIVE REGISTRY" ? (
                        ...
                        >
                            {analyticsData.map((f, i) => (
                                <div key={i} className="bg-white rounded-[8px] border border-primary/5 shadow-sm hover:shadow-2xl transition-all duration-500 overflow-hidden group relative">
                                    <div className="absolute top-4 right-4 z-10 flex gap-2">
                                        <span className={`px-3 py-1.5 rounded-[8px] text-[10px] font-bold uppercase tracking-wider shadow-lg font-inter ${f.status === 'Open for Booking' ? 'bg-green-500 text-white' : f.status === 'Maintenance' ? 'bg-orange-500 text-white' : 'bg-red-500 text-white'}`}>
                                            {f.status}
                                        </span>
                                    </div>
                                    ...
                                    <div className="p-8">
                                        <div className="flex items-start gap-4 mb-8">
                                            <div className="w-16 h-16 rounded-[8px] overflow-hidden bg-primary/5 border border-primary/5 relative flex-shrink-0 group-hover:scale-110 transition-transform duration-700">
                                        ...
                                        <div className="grid grid-cols-2 divide-x divide-primary/5 py-6 border-y border-primary/5 mb-8 bg-[#FAF9F6]/50 rounded-[8px] px-4 font-inter">
                                            <div>
                                        ...
                                        <div className="flex gap-4">
                                            <button className="flex-1 flex items-center justify-center gap-3 px-4 py-4 border border-primary/10 rounded-[8px] text-primary/60 text-[10px] font-bold uppercase tracking-widest hover:bg-primary/5 transition-all font-inter">
                                                <Activity size={14} className="text-accent" />
                                                EDIT
                                            </button>
                                            <button
                                                onClick={() => setDeleteId(f.name)}
                                                className="flex-1 flex items-center justify-center gap-3 px-4 py-4 bg-red-50 text-red-600 rounded-[8px] text-[10px] font-bold uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all font-inter"
                                            >
                        ...
                        <motion.div
                            ...
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            className="flex flex-col items-center justify-center py-40 text-center space-y-6 bg-white rounded-[8px] border border-primary/5 border-dashed"
                        >
                    ...
                    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                        ...
                        >
                            ...
                            <div className="p-12 text-center space-y-8">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <p className="text-[11px] font-bold text-primary/40 uppercase tracking-widest leading-loose max-w-sm mx-auto font-inter">
                                        Are you certain you wish to DELETE <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded-[8px]">{deleteId}</span> from the heritage registry?
                                ...
                                <div className="flex gap-4 font-inter">
                                    <button
                                        onClick={() => setDeleteId(null)}
                                        className="flex-1 px-8 py-5 border border-primary/5 rounded-[8px] text-[10px] font-bold uppercase tracking-widest text-primary/40 hover:text-primary hover:bg-[#FAF9F6] transition-all"
                                    >
                                        CANCEL Request
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        <span>{isDeleting ? "Purging Registry..." : "DELETE"}</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#FAF9F6] py-5 px-12 border-t border-primary/5 flex items-center justify-between">
                                ...
                                <span className="text-[9px] font-bold text-primary/30 uppercase tracking-widest px-3 py-1 bg-white border border-primary/5 rounded-[8px]">ID: {Math.random().toString(36).substr(2, 9).toUpperCase()}</span>
                            </div>
                        </motion.div>
                    </div>
                )}
