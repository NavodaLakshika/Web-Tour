...
export default function AdminRequests() {
    ...
    return (
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            {/* List */}
            <div className="bg-white rounded-[8px] border border-primary/5 shadow-sm overflow-hidden">
                <div className="p-8 border-b border-primary/5 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="relative group flex-1">
                        ...
                        <input
                            ...
                            className="w-full bg-[#FAF9F6] border border-primary/5 rounded-[8px] pl-10 pr-4 py-3.5 text-xs font-bold uppercase tracking-wider outline-none focus:ring-4 focus:ring-accent/10 focus:border-accent/40 transition-all shadow-sm"
                        />
                    </div>
                </div>
                        ...
                        <tbody className="divide-y divide-primary/5">
                            {filteredRequests.map((req, i) => (
                                <tr key={i} className="hover:bg-primary/[0.01] transition-colors group">
                                    ...
                                    <td className="p-6">
                                        <div className="flex flex-wrap gap-2">
                                            {req.accommodation?.map((acc: string, idx: number) => (
                                                <span key={idx} className="px-2 py-1 bg-primary/5 rounded-[8px] text-[9px] font-black text-primary/60 uppercase">{acc}</span>
                                            ))}
                                        </div>
                                    </td>
                                    <td className="p-6">
                                    ...
                                    <td className="p-6">
                                        <span className={`px-3 py-1.5 rounded-[8px] text-[9px] font-black uppercase tracking-widest ${req.status === 'pending' ? 'bg-orange-50 text-orange-600' :
